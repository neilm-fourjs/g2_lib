...
-- Do we have an update server defined?
	...
	DISPLAY "Stat:", l_stat, " Reply:", g2_gdcUpdateCommon.m_ret.reply, " ReplyStat:", g2_gdcUpdateCommon.m_ret.stat
...
-- We have a new GDC Update ! confirm with user
	IF g2_core.g2_winQuestion(
					"GDC Update", SFMT(%"%1\nUpdate Now?", g2_gdcUpdateCommon.m_ret.reply), "Yes", "Yes|No", "question")
			= "No" THEN
		RETURN
	END IF
...
-- does the GDC update file exist on our server
	...
	IF NOT os.Path.exists(l_localFile) THEN
		IF g2_gdcUpdateCommon.m_ret.upd_url IS NOT NULL THEN
			...
			IF NOT getGDCUpdateZipFile(l_localFile, g2_gdcUpdateCommon.m_ret.upd_url, g2_gdcUpdateCommon.m_ret.upd_file) THEN
...
--------------------------------------------------------------------------------
-- Do the web service REST call to check for a new GDC
PRIVATE FUNCTION useGDCUpdateWS(l_url STRING) RETURNS()
