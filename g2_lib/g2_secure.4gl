...
#+
#+ Two hash types are available, see g2_getHashType():
#+
#+ BCRYPT ( the default ):
#+	password hashed with bcrypt ( Blowfish )
#+	you only need to store the Password hash ( bcrypt includes the salt in the hash )
#+
#+ SHA512:
#+	password hashed with SHA512 ( with salt + multiple iterations )
#+	you need to store the Salt and the Password hash
#+
#+ WARNING: the two hash types are NOT compatible - so if you change the hash type
#+ you WILL have to regenerate all the password hashes for your existing accounts!
#+
...
FUNCTION g2_genSalt(l_hashtype STRING) RETURNS STRING
	...
	CASE l_hashtype
		WHEN "BCRYPT"
			CALL g2_init.g2_log.logIt("Generating BCrypt Salt")
			TRY
				LET l_salt = security.BCrypt.GenerateSalt(12)
			CATCH
				CALL g2_init.g2_log.logIt("ERROR:" || status || ":" || sqlca.sqlerrm)
			END TRY
		WHEN "SHA512"
			CALL g2_init.g2_log.logIt("Generating Random Salt")
			LET l_salt = security.RandomGenerator.CreateRandomString(16)
		OTHERWISE
			CALL g2_core.g2_errPopup(%"Unsupported Encryption Type Requested!")
			EXIT PROGRAM
	END CASE
	CALL g2_init.g2_log.logIt("Salt Generated:" || l_salt || " (" || l_salt.getLength() || ")")
	RETURN l_salt
END FUNCTION
--------------------------------------------------------------------------------
...
#+ @param  l_hashtype - String -The type of hash to use ( can be NULL for default )
#+ @return String - A hash of the password, using BCrypt or SHA512
FUNCTION g2_genPasswordHash(l_pass STRING, l_salt STRING, l_hashtype STRING) RETURNS STRING
	...
	TRY
		CASE l_hashtype
			WHEN "BCRYPT"
				CALL g2_init.g2_log.logIt("Generating BCrypt HashPassword")
				LET l_hash = security.BCrypt.HashPassword(l_pass, l_salt)
			WHEN "SHA512"
				CALL g2_init.g2_log.logIt("Generating " || l_hashtype || " HashPassword")
			...
			OTHERWISE
				...
				EXIT PROGRAM
		END CASE
		CALL g2_init.g2_log.logIt("Hash created:" || l_hash || " (" || l_hash.getLength() || ")")
	CATCH
		CALL g2_init.g2_log.logIt("ERROR:" || status || ":" || sqlca.sqlerrm)
	END TRY
...
#+ @param  l_hashtype - String -The type of hash to use ( can be NULL for default )
#+ @return boolean
FUNCTION g2_chkPassword(l_pass STRING, l_passhash STRING, l_salt STRING, l_hashtype STRING) RETURNS BOOLEAN
	DEFINE l_hash STRING
	...
	CASE l_hashtype
		WHEN "BCRYPT"
			CALL g2_init.g2_log.logIt("checking password using BCRYPT")
			TRY
				IF security.BCrypt.CheckPassword(l_pass, l_passhash) THEN
					CALL g2_init.g2_log.logIt("Password checked okay.")
					RETURN TRUE
				END IF
			CATCH
				CALL g2_init.g2_log.logIt("ERROR:" || status || ":" || sqlca.sqlerrm)
			END TRY
		WHEN "SHA512"
			CALL g2_init.g2_log.logIt("checking password using " || l_hashtype)
	...
	CALL g2_init.g2_log.logIt("Password checked failed.")
	RETURN FALSE
END FUNCTION
--------------------------------------------------------------------------------
...
# therefore it is recommended to remove unnecessary whitespace.
	...
	TRY
		...
		ELSE
			CALL g2_init.g2_log.logIt("No encrypted l_node found")
			EXIT PROGRAM 210
		END IF
		...
		IF l_list.getCount() != 1 THEN
			CALL g2_init.g2_log.logIt("Key name doesn't match")
			EXIT PROGRAM 211
		END IF
	CATCH
		CALL g2_init.g2_log.logIt("Unable to load / process XML file :" || status || ":" || err_get(status))
		EXIT PROGRAM 212
	END TRY
	...
	CATCH
		CALL g2_init.g2_log.logIt("Unable to decrypt XML file :" || status || ":" || err_get(status))
		EXIT PROGRAM 213
	END TRY
...
FUNCTION g2_updCreds(l_typ STRING, l_user STRING, l_pass STRING) RETURNS BOOLEAN
	...
	TRY
		...
		IF NOT os.Path.rename(m_file, m_file || l_dte) THEN
			CALL g2_init.g2_log.logIt("Failed to backup creds file:" || status || ":" || err_get(status))
			RETURN FALSE
		END IF
	...
	CATCH
		CALL g2_init.g2_log.logIt("Unable to encrypt XML file :" || status || ":" || err_get(status))
		RETURN FALSE
	END TRY
	RETURN TRUE
END FUNCTION
...
FUNCTION g2_saveSession(l_id STRING, l_user STRING) RETURNS()
	...
	IF l_val IS NOT NULL THEN
		CALL g2_init.g2_log.logIt(SFMT("Save Session id=%1 val=%2", l_id, l_val))
...
FUNCTION g2_getSession(l_id STRING, l_age INTEGER) RETURNS STRING
	...
	IF NOT os.Path.readable(C_PRIVATEKEY) THEN
		CALL g2_core.g2_winMessage("Warning", "Private key file can not be read!\nAuto login not available.", "information")
		RETURN NULL
	END IF
	CALL m_enc.init(C_CERTFILE, C_PRIVATEKEY)
	TRY
	...
	CATCH
		...
		RETURN NULL
	END TRY
	CALL g2_init.g2_log.logIt(SFMT("Get Session id=%1 val=%2", l_id, l_val))
	IF l_val IS NULL THEN
		RETURN NULL
	END IF
	LET l_val = m_enc.decrypt(l_val)
--	CALL g2_init.g2_logIt(SFMT("Get Session val=%1",l_val))
...
PRIVATE FUNCTION get_credFile() RETURNS()
	...
	IF NOT os.Path.exists(m_file) THEN
		CALL g2_init.g2_log.logIt("Creditials File is Missing!")
	ELSE
		CALL g2_init.g2_log.logIt("Creditials File is " || m_file)
	END IF
END FUNCTION
