...
MAIN
	...
	MENU
		COMMAND "Lookup 1 - Colours"
			CALL colours1()
		COMMAND "Lookup 1 - Countries"
			CALL countries1()
		COMMAND "Lookup 2 - Colours"
			CALL colours2()
		COMMAND "Lookup 2 - Countries"
			CALL countries2()
		COMMAND "Lookup 2 - Customers"
			CALL customers2()
		COMMAND "Quit"
			EXIT MENU
		ON ACTION CLOSE
...
FUNCTION colours1()
	...
	LET l_colr = g2_lookup.g2_lookup("colours", "colour_key,colour_name,colour_hex", "Key,Name,HEX", "1=1", "colour_name")
	CALL lib_expect.showResult("Colour:" || NVL(l_colr, NULL))
END FUNCTION
----------------------------------------------------------------------------------------------------
FUNCTION countries1()
	DEFINE l_cntry STRING
	LET l_cntry = g2_lookup.g2_lookup("countries", "country_Code,country_name", "Code,Country", "1=1", "country_name")
...
FUNCTION customers2()
	...
	LET l_lookup.sql_getData =
			"SELECT customer.customer_code, customer.customer_name, addresses.line1 FROM customer, addresses WHERE customer.del_addr = addresses.rec_key ORDER BY customer_name"
